...
interface SpritePetInstanceProps {
  ...
  sceneSpritePets: Array<{instanceId: string; petId: string; x: number; y: number}>;
}
...
const SPECIES_SIZE_FACTOR: Record<string, number> = {
  farm_frog: 0.74,
  farm_littlefox: 0.62,
  farm_littlewhite: 0.88,
  farm_littleblue: 0.88,
  farm_littlegray: 0.88,
  farm_robotbird: 0.88,
  farm_robotsheep: 0.88,
  farm_robotfrog: 0.88,
  farm_robotpig: 0.88,
};
...
const SpritePetInstance: React.FC<SpritePetInstanceProps> = ({pet, mini, onMove, sceneSize, sceneSpritePets}) => {
  ...
  const sceneSpritePetsRef = useRef(sceneSpritePets);
  ...
  useEffect(() => {
    sceneSpritePetsRef.current = sceneSpritePets;
  }, [sceneSpritePets]);
  ...
  return (
    <div
      className="absolute whitespace-nowrap transition-[left,top] ease-in-out"
      style={{
        left: `${pos.x}%`,
        top: `${pos.y}%`,
      ...
      <div className="relative">
        <SpriteActor
          ...
          className="relative z-10 drop-shadow-[0_5px_8px_rgba(15,23,42,0.18)]"
        />
      </div>
    </div>
  );
...
export function PetScene({mini = false}: {mini?: boolean}) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      />
      <div ref={sceneRef} className="absolute inset-x-0 top-0 bottom-24 overflow-hidden">
          ...
          if (isPetSpriteKey(pet.petId)) {
            ...
            return (
              <SpritePetInstance
                ...
                sceneSize={sceneSize}
                sceneSpritePets={sceneSpritePets}
              />
            );
          }
